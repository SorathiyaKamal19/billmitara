...
export function AppLayout() {
  ...
  const [dark, setDark] = useState(
    () =>
      localStorage.getItem("poss_theme") === "dark" ||
  ...
  const restaurantName =
    user?.restaurant?.name || t("Restaurant", "Restaurant");
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-white/50 bg-white/75 p-4 backdrop-blur-xl dark:border-white/10 dark:bg-gray-950/70 lg:block">
        <div className="flex h-full flex-col">
          <div className="rounded-lg bg-ink p-4 text-white">
            <NavLink
              to="/"
              className="flex items-center gap-3 rounded-lg transition hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-white/40"
            >
          ...
          <div className="mt-3 shrink-0 space-y-3 border-t border-gray-200 pt-3 dark:border-white/10">
            <div className="block rounded-lg border border-gray-200 bg-white p-3 transition hover:border-saffron/40 dark:border-white/10 dark:bg-white/10">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                Copyright {currentYear} Bill Mitra. All rights reserved.
              </p>
            </div>
            <div className="flex gap-2">
              ...
              <button
                ...
                title={
                  dark
                    ? t("લાઇટ મોડ", "Light mode")
                    : t("ડાર્ક મોડ", "Dark mode")
                }
                aria-label={
                  dark
                    ? t("લાઇટ મોડ", "Light mode")
        ...
        <div className="mx-auto max-w-7xl">
              ...
              <div className="absolute right-0 top-full z-50 mt-2 w-64 rounded-lg border border-gray-200 bg-white p-3 shadow-soft dark:border-white/10 dark:bg-gray-950">
                ...
                >
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="block truncate text-xs font-semibold capitalize text-gray-500 dark:text-gray-400">
                      {user?.role}
                    </span>
                  </span>
                </button>
          ...
          <nav
            className={clsx(
              ...
              mobileAllowed.length <= 2
                ? "grid-cols-2"
                : mobileAllowed.length === 3
                  ? "grid-cols-3"
                  : mobileAllowed.length === 4
                    ? "grid-cols-4"
                    : "grid-cols-5",
            )}
          >
            {mobileAllowed.map(({ to, labelGu, labelEn, icon: Icon }) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  clsx(
                    "grid place-items-center gap-1 rounded-lg px-2 py-2 text-[11px] font-bold",
                    isActive ? "bg-saffron text-white" : "text-gray-500",
                  )
                }
              >
                <Icon size={18} />
          ...
          {showWelcomeModal && user && (
            <Modal
              title={t("Bill Mitra માં આપનું સ્વાગત છે", "Welcome to Bill Mitra")}
              description={t(
                "તમારું રેસ્ટોરન્ટ વર્કસ્પેસ તૈયાર છે.",
                "Your restaurant workspace is ready.",
              )}
              icon={<Sparkles size={22} />}
              onClose={closeWelcomeModal}
              closeLabel={t("બંધ કરો", "Close")}
              footer={
                <button type="button" className="btn-primary" onClick={closeWelcomeModal}>
                  {t("શરૂ કરો", "Get started")}
                </button>
              }
            >
              <div className="space-y-3 text-sm leading-6 text-gray-600 dark:text-gray-300">
                <p>
                  {t(
                    "હવે તમે ટેબલ, મેનુ, સ્ટાફ અને બિલિંગ સેટિંગ્સ ગોઠવી શકો છો.",
                    "You can now set up tables, menu items, staff accounts, and billing settings.",
                  )}
                </p>
                <p className="font-bold text-gray-950 dark:text-white">
                  {t(
                    "સરસ શરૂઆત માટે પહેલા રેસ્ટોરન્ટ સેટિંગ્સ તપાસો.",
                    "For the best start, review your restaurant settings first.",
                  )}
                </p>
              </div>
            </Modal>
